...
.do_turn
	...
	ld a, [wLastPlayerMon]
	ld [wCurBattleMon], a
	call PlayerMonFaintHappinessMod ; had to change to the old label
	pop af
	ld [wCurBattleMon], a
	call PlayerMonFaintedAnimation
	ld hl, BattleText_MonFainted
...
.done
	...
	and PP_MASK ; should be "and PP_MASK" (fixed)
	ret nz
...
; Try again if length >= 1616 mm (i.e. if LOW(length) >= 3 inches)
	...
	cp 4 ; should be "cp 3", since 1616 mm = 5'3", but LOW(1616) = 80
	jr nc, .GenerateDVs
...
; Try again if length >= 1600 mm (i.e. if LOW(length) >= 2 inches)
	ld a, [wMagikarpLength + 1]
	cp 3 ; should be "cp 2", since 1600 mm = 5'2", but LOW(1600) = 64
	jr nc, .GenerateDVs
